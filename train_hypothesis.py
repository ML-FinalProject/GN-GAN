...
# torchvision >=0.13 compatibility for load_state_dict_from_url
try:
...
# optimizer choice flags
flags.DEFINE_bool('sgd', False, 'use SGD optimizer (no EMA)')
flags.DEFINE_bool('sgd_ema', False, 'use SGD optimizer with EMA')
flags.DEFINE_bool('sgd_ema_dlr', False, 'use SGD + EMA + dynamic LR schedule')
flags.DEFINE_bool('rmsprop', False, 'use RMSprop optimizer')
flags.DEFINE_float('momentum', 0.9, 'momentum for SGD/RMSprop')
flags.DEFINE_float('weight_decay', 0.0, 'weight decay for all optimizers')

# dynamic LR flags (only used when --sgd_ema_dlr is True)
flags.DEFINE_integer('lr_warmup_steps', 0,
                     'warmup steps for dynamic LR (sgd_ema_dlr only)')
flags.DEFINE_float('lr_min_mult', 0.0,
                   'minimum LR multiplier at the end of training (sgd_ema_dlr only)')
...
def train():
    # decide optimizer from flags
    num_opt_flags = (
        int(FLAGS.sgd) +
        int(FLAGS.sgd_ema) +
        int(FLAGS.sgd_ema_dlr) +
        int(FLAGS.rmsprop)
    )
    if num_opt_flags > 1:
        raise ValueError("Choose only one of --sgd, --sgd_ema, "
                         "--sgd_ema_dlr, --rmsprop")
    ...
    if FLAGS.sgd:
        ...
        opt_name = 'sgd_ema'
    elif FLAGS.sgd_ema_dlr:
        opt_name = 'sgd_ema_dlr'
    elif FLAGS.rmsprop:
        opt_name = 'rmsprop'
    else:
        opt_name = 'adam'  # default

    # EMA behaviour: plain sgd → no EMA; others use FLAGS.ema_decay
    if opt_name == 'sgd':
        ema_decay = 0.0
    else:
        ema_decay = float(FLAGS.ema_decay)

    # history for this run (for future visualization)
    history = {
        'optimizer': opt_name,
        'ema_decay': ema_decay,
        'steps': [],
    ...
    # optimizer
    betas = tuple(FLAGS.betas)  # absl gives list
    ...
    elif opt_name in ['sgd', 'sgd_ema', 'sgd_ema_dlr']:
        optim_G = optim.SGD(
            net_G.parameters(),
            lr=FLAGS.lr_G,
    ...
    # scheduler
    if opt_name == 'sgd_ema_dlr':
        # dynamic LR: warmup + cosine decay
        warmup = max(FLAGS.lr_warmup_steps, 0)
        min_mult = float(FLAGS.lr_min_mult)

        def decay_rate(step):
            # step >=1
            if step <= warmup and warmup > 0:
                # linear warmup from 0 → 1
                return max(step, 1) / float(warmup)
            # cosine decay from 1 → min_mult
            effective_start = warmup
            total_span = max(FLAGS.total_steps - effective_start, 1)
            t = (step - effective_start) / float(total_span)
            t = max(0.0, min(1.0, t))
            cos_factor = 0.5 * (1.0 + math.cos(math.pi * t))  # 1 → 0
            return min_mult + (1.0 - min_mult) * cos_factor
    ...
    D_size = sum(p.data.nelement() for p in net_D.parameters())
    G_size = sum(p.data.nelement() for p in net_G.parameters())
        ...
        for step in pbar:
            ...
            for _ in range(FLAGS.n_dis):
                ...
                if FLAGS.cr > 0:
                    ...
                    loss_cr = torch.tensor(0., device=device)
                loss_all = loss + FLAGS.cr * loss_cr
                loss_all.backward()
                optim_D.step()

                loss_sum += float(loss.detach().cpu().item())
                loss_real_sum += float(loss_real.detach().cpu().item())
                loss_fake_sum += float(loss_fake.detach().cpu().item())
                loss_cr_sum += float(loss_cr.detach().cpu().item())
            ...
            with module_no_grad(net_D):
                ...
                loss_G = loss_fn(pred_fake)
                loss_G.backward()
                optim_G.step()

            # ema
            if step < FLAGS.ema_start or ema_decay == 0.0:
                decay = 0.0
            else:
                decay = ema_decay
            ...
            if step == 1 or step % FLAGS.eval_step == 0:
                ...
                history['IS'].append(IS)
                history['FID'].append(FID)
                history['IS_EMA'].append(IS_ema)
                history['FID_EMA'].append(FID_ema)
    ...
    # save full metric history for this run
    history_path = os.path.join(FLAGS.logdir, 'metrics_history.json')
...
def train():
    # decide optimizer from flags
    num_opt_flags = int(FLAGS.sgd) + int(FLAGS.sgd_ema) + int(FLAGS.rmsprop)
    if num_opt_flags > 1:
        raise ValueError("Choose only one of --sgd, --sgd_ema, --rmsprop")
